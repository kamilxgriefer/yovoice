...
eight next-build branches (2026-09-19/20), then in the eight branches built
from Kamil's tester reports on 2026-09-25, then in the build 36 review round
(2026-09-26). 3.0.0+35, merged in on 2026-09-25, is sounds only and fixed none
of them. **None of it is on `main`, none of it is deployed and none of it is
with testers**, so every defect below is still live for every user until
build 36 ships. Deploy order and owner steps:
...
and [Sessions/2026-09-25-build-36.md](Sessions/2026-09-25-build-36.md). Where an
entry below says UNVERIFIED, it has not been looked at on a device or a
simulator — nothing in this build was.

### FIXED IN DOCS — the next-build deploy order would have moved thirteen held functions (2026-09-26, build 36 documentation pass)

The next-build order told the operator to deploy "the whole surface" with
`firebase deploy --only functions` and to expect thirteen creations. On this
source that command creates sixteen: the three Reel voice-comment exports
production has never had (`reserveReelVoiceCommentDraft`,
`finalizeReelVoiceCommentDraft`,
`expireAbandonedReelVoiceCommentDraftsSchedule`) as well. It would also have
moved the nine reader functions pinned on their 2026-09-08 revision until that
slice ships — `getReelViewV2` alone switches the Yeel microphone on against
absent publish callables — and redeployed `acceptDirectCall` with the
`minInstances: 1` the owner declined (ADR-197), a billed warm instance. The
standing rule in DEPLOYMENT.md ("never bare `--only functions`") already
forbade it. Fixed in the build 36 order: one name-scoped selector generated by
module evaluation, 259 names plus `onNotificationCreated` in step 3a, thirteen
held, with its expected count and SHA-256 pinned. Nothing was ever deployed
with the old order.

### FIXED IN SOURCE — the Apple App Review demo account was counted as a person on the website, and Premium still said Servers were "after launch" (2026-09-25, build 36, `stats-exclusion`)

`publicStats/live.activeAccounts` (the website's live account number) is a
`count()` over `publicProfiles`, so the App Review demo account counted as a
user; the 13 Claude test accounts deleted on 2026-09-25 had inflated it the
same way (43 → 34 after that clean-up). `functions/stats/public_stats.js` now
subtracts an explicit, commented list of service accounts
(`PUBLIC_STATS_EXCLUDED_ACCOUNT_UIDS`; only the App Review account today),
only for uids whose `publicProfiles` document exists, de-duplicated and
floored at zero; the account keeps its projection and keeps working for App
Review. The Premium screen said Server allowances apply "after launch" /
"when live" although Servers have been open to everyone since 16 September
2026; English and Polish copy corrected, numbers and entitlements unchanged
(only the finders pinning that exact copy moved). Takes effect with the
`publishPublicStatsSchedule` deploy and the next app build. The account's
address stays in the history of unpushed commit `ebfb04a1` (see the review
round below).
...
- **Storage objects outside a uid-keyed prefix.** `uidStoragePrefixes` is a
  ...
  DEPLOYMENT.md, build 36 deploy order, step 4. Line numbers below are `main`'s.)*
