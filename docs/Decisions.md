...
- The brief's makieta of a voice row with avatars and a head count **before**
  ...
  `channelSessions` are closed to the client (ADR-177), so every face there
  would be invented. The primitive enforces it rather than trusting callers —
  `participants` is dropped unless `connected`.
- The speaking ring is `AppColors.success`, as the brief names it twice, and
  the servers shell's other speaking marks (`ServerParticipantTile`, the
  connected glyph, the family lounge border, the community and meeting stages)
  stay `palette.audioAccent`. That is deliberately two inks for one word and it
  is a debt, not a decision to repeat: phase 2 (Serwery) converges them with
  rendered evidence from the stage, which this phase does not produce. The
  connected marker in the row itself kept `audioAccent` so the row and the dock
  above it still agree.
- The seed preview rows in `create_server_screen.dart` (18 px glyph, tight
  rhythm, non-interactive, inside a card) are a *template preview*, not a
  channel a person can open, and were left alone: forcing them into a 48 px
  `ListTile` would redraw the creation flow, which is not this family's
  boundary. The clubs `_ChannelTile` (`clubs/…/club_overview_screen.dart`) is
  a `ClubChannel` row on a route retired from active routing
  (`test/server_surface_cutover_test.dart`); it is not deleted ("never remove
  functionality") and not migrated, so the phase-0 commit stays inside the
  servers feature.
- `serverChannelIcon` stays exported from `server_panel.dart`: 23 call sites
  in eleven other files of the servers feature import it from there, and the
  family's job was to remove the *second* map, not to move the first.
- Visible normalisations in the management sheet, none of them pinned by a
  test: its rows now wear the panel's glyphs (9 of 13 kinds change), the lock
  for a restricted channel, `AppTypography.bodyMedium`, 12 px content padding
  instead of Material's 16 and a rounded splash. The `server-management-channel-<id>`
  key, the 56 px floor, the access subtitle, the `PopupMenuButton` and every
  action are untouched.
- `minTileHeight` stays 48 although "Smuklej" asks for 56–68: raising it now
  would move the phone sheet at 320 px / 200 % (`server_independent_qa_layout_test`)
  and every server frame captured so far. Phase 2 raises it with evidence.
- The row-level `Dołącz` is an icon control with the join label as tooltip and
  semantics, not a worded button. A worded one is what the makieta draws, but
  at 320 px / 200 % text `Dołącz do rozmowy` beside the name and the marker
  leaves neither readable; the word itself is never lost, because the channel
  scene's `server-join` still spells it.
- Nothing was removed from `ServerPanel`: `connectedChannelId` survives beside
...
## ADR-XXX: A Google or Apple sign-in that inherits a password nobody verified ends every earlier session (pre-registered account takeover, Phase 1)

**Date:** 2026-09-25 · **Status:** accepted (source on branch
`nb2-account-takeover`: `a136ea2a` server, rules and tests; `1c248efb` app;
the commit after them adds these docs and strips address-less identities) ·
...
1. **One remediation authority**, `remediateAccount()` in
   `functions/auth/federated_takeover.js`, used by every trigger. It acts only
   when the account has a Google/Apple identity on the account's own address
   AND either still carries an unverified password, or its password vanished
   while the server-side ledger still says "pending". A password its holder
   verified is the owner's by definition and is never touched — including the
   race where a verified password meets Google before any sweep has seen the
   verification ("ambiguous": logged, left alone).
2. **Evidence is recorded while it is visible.** `authPasswordLedger/{uid}`
   ...
   accounts and re-arms an address that became unverified later.
3. **Order**, so nothing planted or issued survives in between: (1) unlink the
   password and every identity that is not the owner's, (2) revoke refresh
   tokens, (3) write `users/{uid}.authSessionEpoch` = remediation second + 1,
   (4) purge `fcmTokens` (all of them from the sweeper; all but the calling
   device's for the owner, and only when the caller's session is the kept
   identity, fresh and the account's latest sign-in), (5) audit row in
   `authTakeoverAudit` plus ledger `remediated`, one batch. Unlinking first
...
4. **Two triggers.** A: `secureFederatedSignInV1`, called by the owner's own
   client right after a returning Google/Apple sign-in; it remediates the
   caller only, answers `notApplicable` to any non-federated session, and is
   best-effort on the client (a failure never blocks a sign-in). The app wires
   it in `AuthService` and, on `remediated`, signs out and asks the owner to
   sign in once more (localized notice), because revocation ended this
   device's session too. B: `sweepFederatedTakeoverSchedule`, every 5 minutes,
   `maxInstances: 1`, time-bounded, for the website and old builds.
...
- The owner signs in once more after a remediation. On the website and old
  builds the sweeper acts within 5 minutes and that session ends at its next
  refresh.
...
- Cost: one Firestore read and one Auth lookup per returning federated
  sign-in; one indexed equality query every 5 minutes; the `listUsers` walk
...
- **Website (integrator; the website repo was not edited).** In
  ...
  export async function secureReturningFederatedSignIn(
    ...
    try {
      ...
      status = (await check({})).data?.status;
    } catch (error) {
  ...
  `if (error instanceof FederatedSessionSecuredError) return "We secured your account and ended every earlier session, including a password sign-in that was never verified. Sign in again to continue.";`
