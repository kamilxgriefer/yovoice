...
- `sendServerEventRemindersSchedule` (`functions/notifications/server_events.js`)
  ...
  **paged with a cursor** until exhausted or until a 240 s wall-clock budget
  ...
  backpressure (ADR-213). The notification id carries the event revision, but
...
- `createServerRolePromotionNotifier`
  ...
  per-actor-per-recipient-per-role budget (1 per 24 h, ADR-213): a cycled
...
**two-valued** (ADR-213): a genuinely stale source is cleaned up, while an
