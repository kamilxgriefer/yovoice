...
function requireExactInput(data) {
  if (!data || typeof data !== "object" || Array.isArray(data)) {
    throw new HttpsError(
      "invalid-argument",
      "A pinned Moment selection is required.",
    );
  }
  ...
  if (
    ...
    throw new HttpsError(
      "invalid-argument",
      "The selected Voice Moment is invalid.",
    );
  }
...
function activeCreatorEntitlement(data, nowMs) {
  return (
    paidPremiumIsActive(data, nowMs) &&
...
function eligibleMoment(data, uid) {
  const hasCanonicalMedia =
    typeof data?.storagePath === "string" &&
    data.storagePath.length > 0 &&
    typeof data?.mediaGeneration === "string" &&
    data.mediaGeneration.length > 0 &&
    (data?.audioUrl === null ||
      (typeof data?.audioUrl === "string" && data.audioUrl.length > 0));
  // Keep the pin projection compatible until the integrity migration has
  // rewritten every legacy Moment. Playback never trusts this durable URL;
  // the client resolves media through the authorized callable instead.
  ...
  return (
    ...
    (hasCanonicalMedia || hasLegacyMedia) &&
...
function createPinnedPostsService({
  ...
  if (!Number.isSafeInteger(attemptWindowMs) || attemptWindowMs < 1_000) {
    throw new TypeError("attemptWindowMs must be at least one second.");
  }

  async function consumePinnedPostAttempt(uid, nowMs, now) {
    const scope = "creator.pinnedPost.attempt";
    const rateRef = rateLimitReference(firestore, scope, uid);
  ...
  async function setCreatorPinnedPost(request) {
    ...
    const uid = auth.uid;
    // Commit an actor-wide budget before reading any caller-selected Moment.
    // A denied target transaction would roll its own quota write back and let
    // random ids create unlimited free Firestore reads.
    await consumePinnedPostAttempt(uid, nowMs, now);
    ...
    return firestore.runTransaction(async (transaction) => {
      ...
      const momentRef =
        momentId === null ? null : firestore.doc(`voiceMoments/${momentId}`);
      ...
      if (
        ...
        !eligibleMoment(momentSnapshot.data(), uid)
      ) {
        throw new HttpsError(
          "failed-precondition",
      ...
      const pinnedAt =
        previous.momentId === momentId && previous.pinnedAt
          ? previous.pinnedAt
          : now;
  ...
  async function clearPinForIneligibleMoment(momentId) {
    ...
    return firestore.runTransaction(async (transaction) => {
      ...
      const moment = momentSnapshot.exists
        ? (momentSnapshot.data() ?? {})
        : null;
      const creatorId =
        typeof moment?.authorId === "string" && moment.authorId
          ? moment.authorId
          : null;
  ...
  async function clearDeletedMomentPin(momentId, creatorId) {
    if (!SAFE_DOCUMENT_ID.test(momentId) || !canonicalCreatorId(creatorId)) {
      return { cleared: false };
    }
    const pinRef = firestore.doc(`creatorPinnedPosts/${creatorId}`);
    return firestore.runTransaction(async (transaction) => {
      const [momentSnapshot, pinSnapshot] =
        typeof transaction.getAll === "function"
      ...
      if (
        momentSnapshot.exists &&
        eligibleMoment(momentSnapshot.data(), creatorId)
      ) {
        return { cleared: false };
      }
      if (!pinSnapshot.exists || pinSnapshot.data()?.momentId !== momentId) {
...
async function handlePinnedMomentEligibilityChanged(
  ...
  const beforeCreatorId =
    typeof beforeData?.authorId === "string" ? beforeData.authorId : null;
  const afterCreatorId =
    typeof afterData?.authorId === "string" ? afterData.authorId : null;
  const outcomes = [];
