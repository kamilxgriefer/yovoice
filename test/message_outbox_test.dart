...
void main() {
  ...
  Future<OutboxEntry> enqueueOne(MessageOutbox outbox, String text) =>
      outbox.enqueue(
        conversationId: 'c1',
  ...
  group('persistence', () {
    test('a queued message survives a restart, with its requestId', () async {
      ...
      expect(
        restored.requestId,
        queued.requestId,
        reason: 'a restart that changed the requestId would turn the next '
    ...
    test('one unreadable entry does not strand the rest of the queue',
        () async {
      ...
      final stored = jsonDecode(prefs.getString('messages.outbox.v1')!) as List;
  ...
  group('states', () {
    ...
    test('a transient failure moves it to Retrying and counts the attempt',
        () async {
      final outbox = await outboxWith();
    ...
    test('the retry budget is bounded — it becomes Failed rather than '
      ...
      expect(
        outbox.due(),
        isEmpty,
        reason: 'a terminal entry is never picked up by the automatic loop '
            'again; an endless retry is a battery and quota drain',
      );
    });

    test('a refusal fails immediately, without spending the retry budget',
        () async {
      final outbox = await outboxWith();
    ...
    test('a manual retry resets the attempts but KEEPS the requestId',
        () async {
      ...
      expect(
        revived.requestId,
        entry.requestId,
        reason: 'a manual retry of a send that secretly succeeded must still '
  ...
  group('bounded', () {
    test('it refuses past capacity instead of growing without limit',
        () async {
    ...
    test('a queue full of unsent messages is never silently discarded',
        () async {
      ...
      expect(
        ...
        ['one', 'two'],
        reason: 'evicting an unsent message to make room would lose exactly '
