...
void main() {
  test(
    'NO MicState can produce a mute control in a room that is not live',
    () {
  ...
  test(
    'a dormant room never reads as broken audio — "OFFLINE" describes a '
  ...
  test('exactly two states are mute controls and exactly one is a start', () {
    expect(
      RoomMicAffordance.values.where((a) => a.isMuteControl).toSet(),
      {RoomMicAffordance.live, RoomMicAffordance.muted},
    );
