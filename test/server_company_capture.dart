// Developer-only VISUAL capture for the COMPANY template (board 04) on the
// server shell.
//
...
// this harness. It renders the real widgets at the shipping widths with the
// real Inter face and writes PNGs so a human — and the Senior Visual Quality
// Specialist — can look at what the code actually draws. Run explicitly:
//
...
ServerChannelLiveness get _live => ServerChannelLiveness(
  isLive: true,
  startedAt: DateTime.now().subtract(const Duration(minutes: 12, seconds: 24)),
);
...
void main() {
  ...
  for (final (label, size, scale, light)
    ...
    testWidgets('company meeting before a join $label', (tester) async {
      ...
      await _shoot(
        tester,
        captureKey: captureKey,
        name: 'company-meeting-quiet-$label',
      );
    });
  }
    ...
    testWidgets('company meeting in session $label', (tester) async {
      ...
      await _render(
        ...
        child: board.companyWorkspace(
          ...
          connector: connector,
          screenShare: web
              ? ServerScreenShareCapability.web
              : ServerScreenShareCapability.mobile,
        ),
      );
      ...
      await _shoot(
        tester,
        captureKey: captureKey,
        name: 'company-meeting-session-$label',
      );
    });
  }

  testWidgets('company whiteboard view 1440x900', (tester) async {
    final captureKey = GlobalKey();
    final connector = FakeServerMediaConnector();
    await _render(
      tester,
  ...
  testWidgets('company panel search 1440x900', (tester) async {
    final captureKey = GlobalKey();
    await _render(
      tester,
      captureKey: captureKey,
    ...
    await _shoot(
      tester,
      captureKey: captureKey,
      name: 'company-panel-search-1440x900',
    );
  });

  testWidgets('company phone conversation tab 390x844', (tester) async {
    final captureKey = GlobalKey();
    final connector = FakeServerMediaConnector();
    await _render(
      tester,
    ...
    await _shoot(
      tester,
      captureKey: captureKey,
      name: 'company-phone-chat-390x844',
    );
  });
}
