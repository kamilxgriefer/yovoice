...
void main() {
  ...
  group('bell feed and badge', () {
    ...
    test('a legacy document with no bellSuppressed field stays visible',
        () async {
      await db
          .collection('users')
          .doc(me)
  ...
  group('stream ownership', () {
    test('a second account gets its OWN feed, never the previous one\'s',
        () async {
      await seedNotification(id: 'follow_$other', type: 'follow');
      expect((await service.watchNotifications().first).single.id,
          'follow_$other');
    ...
    test('cancelling the feed subscription closes its underlying streams',
        () async {
      await seedNotification(id: 'follow_$other', type: 'follow');
      final received = <int>[];
      final subscription = service
          .watchNotifications()
          .listen((event) => received.add(event.length));
