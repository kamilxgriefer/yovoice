...
void main() {
  ...
  group('web/index.html', () {
    ...
    test(
      'lets Flutter own the only loading screen without an old HTML splash',
      () {
        expect(html, isNot(contains('id="yovoice-bootstrap"')));
        expect(html, isNot(contains('class="boot-wave"')));
        expect(html, isNot(contains('Create your space')));
        expect(html, contains('background-color: #0D0618'));
        expect(
          html,
        ...
        expect(bootstrap, isNot(contains('setTimeout(')));
        expect(bootstrap, isNot(contains('requestAnimationFrame(')));
        expect(bootstrap, isNot(contains('yovoice-bootstrap')));
      },
    );
  });
